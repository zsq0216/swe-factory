...
def build_instance_image_one_stage(
        test_spec: TestSpec,
        client: docker.DockerClient,
        logger: logging.Logger,
        nocache: bool,
        output_path:Path,
        # setup_predictions: dict,
    ):
    """
    Builds the instance image for the given test spec if it does not already exist.
    ...
    # Path(output_path) / test_spec.instance_image_key.replace(":", "__")
    new_logger = False
    if logger is None:
        new_logger = True
        logger = setup_logger(test_spec.instance_id, build_dir / "prepare_image.log")
...
def build_setup_container(
    ...
    try:
        ...
        # config = MAP_VERSION_TO_INSTALL[test_spec.repo][test_spec.version]
        user = "root" 
        # if not config.get("execute_test_as_nonroot", False) else "nonroot"
        # nano_cpus = config.get("nano_cpus")
